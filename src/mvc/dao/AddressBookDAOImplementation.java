...
public class AddressBookDAOImplementation implements AddressBookDAO{
    ...
    {   
       try
       {
          conn = DBConnection.getInstance().getConnect();
           ...
           String qry;
           qry = "insert into AddressBook values ('"+person.getName()+"','"+person.getMob()+"','"+person.getEmail()+"','"+person.getAddress()+"','"+person.getPin()+"')";
    ...
    @Override
     public void updatePerson(Person person,String name)
     {
         try
       {
           conn = DBConnection.getInstance().getConnect();
           st = conn.createStatement();
            String qry="update AddressBook set name='"+person.getName()+"',mob='"+person.getMob()+"',email='"+person.getEmail()+"',address='"+person.getAddress()+"',pin='"+person.getPin()+"' where name='"+name+"'";
           st.executeUpdate(qry);
           conn.close();
        }
    ...
    /*public void searchPerson(Person person,String name)
         ...
         try
       {
          DefaultListModel<String> dlm = new DefaultListModel<>();
           ...
           conn = DBConnection.getInstance().getConnect();
            Statement stmt;
           st = conn.createStatement();
            String qry="select * from addressbook where name like '%u';";
     ...
     public DefaultListModel searchPerson(NameListPanel namePanel,String selectedName) 
    {
             DefaultListModel<String> dlm = new DefaultListModel<>();
            try {
         conn = DBConnection.getInstance().getConnect();
            ...
            String qry = "select * from addressbook where name like 'name%'"+selectedName+"'";
                 try (ResultSet rs = stmt.executeQuery(qry)) {
                     while (rs.next()){
    ...
    public void getSelectedName(DetailViewPanel detailPanel,String selectedName)
    {
        try {
            ...
            while(rs.next()){
             ...
             detailPanel.setPin(rs.getString("pin"));
